# Modliq Admin Console Documentation
...
The **Modliq Admin Console** (`/admin/*`) is the B2B SaaS management control center. Restricted strictly to platform administrators (`user.role === 'ADMIN'`), it provides cross-tenant monitoring, SaaS operational metrics, website CMS controls, AI provider health monitoring, pilot lead management, and audit log analysis.
...
2. **User Account Management** (`/admin/users`): Paginated user directory with role assignments (`ADMIN`, `USER`), account status toggles, organization associations, and Public User ID (`MODLIQ-USER-20260808-1000`) search.
...
12. **Support Ticket Center** (`/admin/support`): Customer support ticket inbox, priority handling, ticket timeline, and staff response editor (`MODLIQ-TICKET-20260808-1000`).
