# MODLIQER User Console Architecture & Features
...
The **MODLIQER User Console** (`/[userId]/modliq-console/*`) is the central workspace where plant operators, process engineers, and quality heads analyze datasets, run ML optimizations, monitor SPC quality statistics, manage operations, track supply chain risks, and generate buyer-ready Quality Passports.
...
2. **Projects & Project Switcher** (`/[userId]/modliq-console/projects`): Project management supporting creation, deletion, renaming, project status badges, and human-readable project IDs (e.g. `MODLIQER-PROJECT-20260808-1000`).
...
13. **Quality Passport** (`/[userId]/modliq-console/quality-passport`): Buyer-ready evidence reports with audit scores, certificate IDs (`MODLIQER-PASSPORT-20260808-1000`), data lineage, Markdown export, and token-hashed public verification links.
14. **MODLIQER Agent (Beta)** (`/[userId]/modliq-console/agent`): Autonomous manufacturing assistant operating across 6 specialized modes (Data Analyst, ML Engineer, Quality Engineer, Operations, Supply Chain, Quality Passport) with human-in-the-loop approval cards for critical actions.
...
17. **Support & Tickets**: Human-readable support ticket tracking (`MODLIQER-TICKET-20260808-1000`).
18. **User Profile & Settings**: Module preferences, profile management, organization details, and public User ID (`MODLIQER-USER-20260808-1000`).
