...
async function main() {
  ...
  console.log('[1] Creating mock lead in Prisma database...');
  let leadRecord;
  try {
    ...
    leadRecord = { id: `lead_mem_${timestamp}`, ...mockLead, createdAt: new Date().toISOString() };
  ...
  const foundLead = allLeads.find((l) => l.email === mockLead.email) || memoryContactLeads.find((l) => l.email === mockLead.email);
