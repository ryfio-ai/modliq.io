...
export default function LaunchCountdownScreen({
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-white text-[#1B2A4A] bg-[radial-gradient(circle_at_top,#F0F6FA,transparent_55%)] font-sans antialiased flex flex-col justify-between select-none relative">
      
      {/* Top Header */}
      <header className="h-16 sm:h-20 shrink-0 border-b border-[#D0E2F0]/80 bg-white/90 backdrop-blur-md px-4 sm:px-8">
        <div className="max-w-6xl mx-auto h-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-[11px] font-extrabold text-[#2B70AB] bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">
              Qeltrava AI
            </span>
          </div>

          <div className="text-xs font-semibold text-slate-500 hidden sm:block tracking-wide">
      ...
      <main className="h-[calc(100vh-8rem)] flex flex-col items-center justify-center px-4 sm:px-6 text-center max-w-4xl mx-auto w-full space-y-3 sm:space-y-4 py-2 z-10">
        ...
        {/* 2. Main Headline */}
        <h1 className="text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-[#1B2A4A] tracking-tight leading-[1.05] max-w-3xl">
          Manufacturing intelligence is about to get simpler.
        </h1>

        {/* 3. Inspirational Founder Line */}
        ...
        {/* 4. Quote Card */}
        <div className="max-w-lg w-full bg-white border border-[#D0E2F0] rounded-2xl px-5 py-3 shadow-[0_8px_24px_rgba(27,42,74,0.06)] text-center shrink-0 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-[#2B70AB]" />
          <blockquote className="text-xs sm:text-sm font-semibold text-[#1B2A4A] italic leading-snug">
            &ldquo;Without data, you&apos;re just another person with an opinion.&rdquo;
          </blockquote>
          <cite className="block text-[10px] sm:text-[11px] font-extrabold text-[#2B70AB] not-italic mt-1 uppercase tracking-wider">
            — W. Edwards Deming
          </cite>
        </div>
      ...
      {/* Footer Attribution */}
      <footer className="h-14 shrink-0 border-t border-[#D0E2F0]/80 bg-white/90 backdrop-blur-md px-4 sm:px-8 text-center text-xs text-slate-500">
        <div className="max-w-6xl mx-auto h-full flex flex-col sm:flex-row items-center justify-between gap-2">
